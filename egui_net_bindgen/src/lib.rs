...
impl BindingsGenerator {
    ...
    fn rename_types(&mut self) {
        for (name, mut value) in std::mem::take(&mut self.registry) {
            let new_name = if let Some((_, new)) = TYPE_RENAMES.iter().find(|(old, _)| *old == name) {
                new.to_string()
            }
            else {
                name
            ...
            match &mut value {
                ContainerFormat::Struct(nameds) => for format in nameds {
                    if let Format::TypeName(name) = &mut format.value {
                        if let Some((_, new)) = TYPE_RENAMES.iter().find(|(old, _)| old == name) {
                            *name = new.to_string();
                        }
                    }
                },
        ...
        for (_, item) in &mut self.krate.index {
            if let Some(name) = &mut item.name && matches!(item.inner, ItemEnum::Struct(_)) {
                if let Some((_, new)) = TYPE_RENAMES.iter().find(|(old, _)| old == name) {
                    *name = new.to_string();
                }
            }
