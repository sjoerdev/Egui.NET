...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
    ...
    .with_byref(EguiFn::egui_context_Context_ref_id, (|ctx: &Context| unsafe { *transmute::<_, &usize>(ctx) }) as fn(&_) -> _)
    .with_byref(EguiFn::egui_context_Context_ref_count, (|ctx: &Context| unsafe { transmute::<_, &&AtomicUsize>(ctx).load(atomic::Ordering::SeqCst) }) as fn(&_) -> _)
    .with_byref(EguiFn::egui_context_Context_ref_decrement, (|ctx: &Context| unsafe { (ctx as *const Context).read(); }) as fn(&_) -> _)
    ...
    .with_byref(EguiFn::egui_containers_popup_Popup_show, (|ctx: &Context, popup: SerializablePopup, mut is_open: bool, callback: EguiCallback| unsafe {
        let response = popup.to_popup(ctx, &mut is_open).show(|ui| callback.invoke(ui as *const _ as *const _));
    ...
    .with(EguiFn::egui_response_Response_mark_changed, (|mut x: Response| { x.mark_changed(); x }) as fn(_) -> _)
    .with(EguiFn::egui_response_Response_mark_changed, (|mut x: Response| { x.set_close(); x }) as fn(_) -> _)
    // Bindings for Sense
