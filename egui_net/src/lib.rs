...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
    ...
    //.with_byref(EguiFn::egui_containers_popup_Popup_show, (|ctx: &Context, popup: SerializablePopup| Popup::position_of_id(ctx, id)) as fn(&_, _) -> _)
...
/*
impl<'a> From<SerializablePopup> for Popup<'a> {
    fn from(value: SerializablePopup) -> Self {
        Self {
            id: value.id,
            anchor: value.anchor,
            rect_align: value.rect_align,
            alternative_aligns: value.alternative_aligns,
            layer_id: value.layer_id,
            close_behavior: value.close_behavior,
            info: value.info,
            kind: value.kind,
            gap: value.gap,
            widget_clicked_elsewhere: value.widget_clicked_elsewhere,
            width: value.width,
            sense: value.sense,
            layout: value.layout,
            frame: value.frame,
